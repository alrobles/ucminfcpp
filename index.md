...
function for general-purpose unconstrained nonlinear optimization.  
...
The original algorithm was written in Fortran by Hans Bruun Nielsen
(IMM,  
...
Fortran-based algorithm described by:
...
integration with R packages. The structure in R draws from the
**FortranCallsR** package by Diethelm Wuertz.
...
| **R interface**        | Drop-in replacement for [`ucminf::ucminf()`](https://rdrr.io/pkg/ucminf/man/ucminf.html) via Rcpp. |
...
is called from R, every evaluation of `fn` and `gr` crosses the
...
- Use
  ...
  for compiled R code.
- For high-dimensional problems (n ≥ 20), optimized C++ kernels offer
  larger gains. \`\`
