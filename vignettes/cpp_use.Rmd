...
The optimizer returns a `ucminf::Result` struct:
...
int main() {
    ...
    ucminf::Result res = ucminf::minimize({-1.2, 1.0}, banana);
...
## Zero-Overhead API: `ucminf::minimize_direct<F>()`
...
int main() {
    ucminf::Result res = ucminf::minimize_direct({-1.2, 1.0}, rosenbrock);
    // ...
    return 0;
}
```
...
Pass a `ucminf::Options` struct to either API to control convergence
...
ucminf::Result res = ucminf::minimize({-1.2, 1.0}, banana, opts);
```
