---
title: "Introduction to ucminfcpp"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to ucminfcpp}
...
Fortran-based [ucminf](https://CRAN.R-project.org/package=ucminf) algorithm.
It is a **drop-in replacement** for `ucminf::ucminf()`.

The package was primarily developed to power the
[`nicher`](https://github.com/alrobles/nicher) and
[`xsdmMle`](https://github.com/alrobles/xsdmMle) packages, where fast and
reliable unconstrained optimization is central to fitting ecological niche
models and maximum-likelihood estimators.
...
```{r compare, eval=requireNamespace("ucminf", quietly = TRUE)}
...
outperforms the Fortran-based `ucminf` due to reduced C/Fortran
call overhead in its C++ core.

```{r benchmark, eval=requireNamespace("microbenchmark", quietly = TRUE) && requireNamespace("ucminf", quietly = TRUE)}
...
it via `ucminfcpp::minimize_direct<F>()` to eliminate all interpreter overhead.
